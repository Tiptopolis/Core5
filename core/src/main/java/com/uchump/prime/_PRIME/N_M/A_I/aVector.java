...
import static com.uchump.prime._PRIME.N_M.Prototype.uVector.*;
...
import com.uchump.prime._PRIME.N_M.UTIL.N_Operator;
...
public class aVector<T extends Number> extends aNumber
	...
	public aVector crs(aVector other) {
		/*Number[] IndexA = N_Resolver.scanFrom(this.elements.toArray());
		Number[] IndexB = N_Resolver.scanFrom(other.elements.toArray());
	...
	/*public aVector prj(aMatrix m) {
		aVector vec = new aVector(this.length(), this.type, false);

		float inv_w = 1.0f / (this.cpy().mul(m.columns()[m.columns().length - 1]).sum());

		for (int i = 0; i < vec.length(); i++) {
			aVector col = new aVector(this.length(), this.type, true);
			if (i <= m.columns().length)
				col = m.columns()[i].cpy();

			vec.setAt(i, this.cpy().mul(col.cpy()).sum() * inv_w);
		}
		return this.set(vec);
	}*/
	...
	/*public aVector rot(aMatrix m) {
		...
		for (int i = 0; i < vec.length(); i++) {
			...
			vec.setAt(i, this.cpy().mul(col.cpy()).sum());
		}
		return this.set(vec);
	}*/
	...
	/*public aVector rotate(final aVector axis, float degrees) {
		aMatrix tmpMat = new aMatrix(4);
		tmpMat.setToRotation(axis, degrees);
		return this.mul(tmpMat);
	}*/

	/*public aVector<T> rotate(float degrees, float axisX, float axisY, float axisZ) {
		aMatrix tmpMat = new aMatrix(4);
	...
	/*public aVector setFromMatrix(aMatrix matrix) {
		return this.setFromMatrix(true, matrix);
	}*/

	/*public aVector setFromMatrix(aMatrix matrix, int len) {
		return this.setFromMatrix(true, matrix, len);
	}*/

	/*public aVector setFromMatrix(boolean normalizeAxes, aMatrix matrix, int len) {
		aMatrix M = uMatrix.boxTo(len, matrix);

		return uVector.setFromAxes(this, normalizeAxes, M.get("M00").floatValue(), M.get("M01").floatValue(),
				M.get("M02").floatValue(), M.get("M10").floatValue(), M.get("M11").floatValue(),
				M.get("M12").floatValue(), M.get("M20").floatValue(), M.get("M21").floatValue(),
				M.get("M22").floatValue());
	}*/

	/*public aVector setFromMatrix(boolean normalizeAxes, aMatrix matrix) {
		// culls it to a 3x3 from the origin
		return this.setFromMatrix(normalizeAxes, matrix, 3);
	}*/
