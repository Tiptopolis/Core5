package com.uchump.prime._RAUM.RAUM.META.NIX;
...
public class _Archetype extends Type {
	...
	public HashMap<String, aProperty> Required = new HashMap<String, aProperty>(0);
	...
	public Array<aProperty> required() {
		Array<aProperty> a = new Array<aProperty>(true, 0, aProperty.class);
		for (aProperty p : this.Required.values()) {
			a.add(p);
		}
		return a;
	}

	public void require(aProperty p)
	{
	...
	public String toLog() {
		...
		if (!this.Required.isEmpty()) {
			for (Entry<String, aProperty> E : this.Required.entrySet()) {
