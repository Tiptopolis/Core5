package com.uchump.prime._RAUM.RAUM.GRID.BAK;
...
public class RectLayerGen {
	...
	public static _Layer genLayer(String label, int width, int height) {
		...
		return genLinks(genTags(genCells(RECT)));
	}
		...
		for (int i = 0; i < L.Size.x; i++) {
			for (int j = 0; j < L.Size.y; j++) {
				...
				_BoundShape b = _BoundShape.defShape(pos.cpy().scl(L.CellSize.cpy()).add(L.CellSize.cpy().scl(0.5f)),
						L.CellSize.cpy().scl(0.7f), new Vector3(0, 0, 0), Color.CLEAR, 4);
	...
	public static _Layer genLinks(_Layer L) {
		...
		for (_Thing C : L.Cells.values()) {
			...
			if (xBound != -1) {
				Node<_Thing> N = new Node("Neighbor", L.Cells.get(new Vector3(((int) p.x) - 1, ((int) p.y), 1)).get());
				C.connect("LEFT", N);
			}
			if (xBound != 1) {
			...
			if (yBound != -1) {
				Node<_Thing> N = new Node("Neighbor", L.Cells.get(new Vector3(((int) p.x), ((int) p.y - 1), 1)).get());
				C.connect("DOWN", N);
			}
			if (yBound != 1) {
				Node<_Thing> N = new Node("Neighbor", L.Cells.get(new Vector3(((int) p.x), ((int) p.y) + 1, 1)).get());
